...
async run(client, message, args) {
    ...
    let av = `https://some-random-api.ml/canvas/blue?avatar=${user.avatarURL({ size: 4096, dynamic: true,format: 'png' })}`    
...
if (res.statusCode !== 200) { //check if it isnt 200 OK
    ...
    let attachment = new MessageAttachment(res.body, `${user.username}_blue.png`) //make attachment using the buffer
